...
template <typename K, typename V>
void realWordTestRoutine(HashTable<K, V>& HashTable, int elements, std::ofstream& file_hash, std::ofstream& file_op, bool mode = true) {
    const int ELEMENTS = elements;
    int errors = 0;

    // Generatore di stringhe casuali
    std::vector<std::pair<std::string, std::string>> words;
    words.reserve(ELEMENTS);
    ...
    while(counter < words.size()) {
        ...
        switch(dist2(gen)) {
            ...
            case 3: {
                // Exist
                if(counter > 0) {
                    std::uniform_int_distribution<> dist_temp(0, words.size() - 1);
                    exist(HashTable, words[dist_temp(gen)].first, file_op, elements, mode);
                }
                break;
            }
            case 4:
            ...
            case 6: {
                // Insert
                insert(HashTable, words[counter], file_op, elements, mode);
                counter++;
                break;
            }
            case 7: {
                ...
                if(counter > 0) {
                    std::uniform_int_distribution<> dist_temp(0, words.size() - 1);
                    update(HashTable, std::make_pair(words[dist_temp(gen)].first, generateRandomString()), file_op, elements, mode);
                }
                break;
            }
            case 8: {
                ...
                if(counter > 0) {
                    std::uniform_int_distribution<> dist_temp(0, words.size() - 1);
                    get(HashTable, words[dist_temp(gen)].first, file_op, elements, mode);
                }
                break;
            }
            case 9: {
                ...
                if(counter > 0) {
                    std::uniform_int_distribution<> dist_temp(0, words.size() - 1);
                    remove(HashTable, words[dist_temp(gen)].first, file_op, elements, mode);
                }
                break;
            }
        }
...
void realWordTest(int elements, std::pair<std::ofstream, std::ofstream>& files, int load_factor = 0) {
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING, HASH::MODE_1, RESIZE::DOUBLE, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
        std::cout << "HashTable:" << std::endl;
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING, HASH::MODE_1, RESIZE::DOUBLE, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second, false);
    }
    /*
    * HashTable:
    *   OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING, HASH::MODE_1, RESIZE::CONSTANT, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING, HASH::MODE_1, RESIZE::DYNAMIC, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_LINEAR_PROBING, HASH::MODE_2, RESIZE::DOUBLE, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_DOUBLE_HASHING, HASH::MODE_1, RESIZE::DOUBLE, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_DOUBLE_HASHING, HASH::MODE_1, RESIZE::CONSTANT, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_DOUBLE_HASHING, HASH::MODE_1, RESIZE::DYNAMIC, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::OPEN_ADDRESSING_DOUBLE_HASHING, HASH::MODE_2, RESIZE::DOUBLE, 256, linear_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CHAINING, HASH::MODE_1, RESIZE::DOUBLE, 256, chaining_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CHAINING, HASH::MODE_1, RESIZE::CONSTANT, 256, chaining_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CHAINING, HASH::MODE_1, RESIZE::DYNAMIC, 256, chaining_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CHAINING, HASH::MODE_2, RESIZE::DOUBLE, 256, chaining_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CUCKOO, HASH::MODE_1, RESIZE::DOUBLE, 256, cuckoo_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CUCKOO, HASH::MODE_1, RESIZE::CONSTANT, 256, cuckoo_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CUCKOO, HASH::MODE_1, RESIZE::DYNAMIC, 256, cuckoo_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
    ...
    */
    {
        ...
        HashTable<std::string, std::string> HashTable(OVERFLOW::CUCKOO, HASH::MODE_2, RESIZE::DOUBLE, 256, cuckoo_load_factor);
        realWordTestRoutine(HashTable, elements, files.first, files.second);
    }
    {
        std::cout << "-----------------------------" <<std::endl;
...
int main() {
    ...
    for(int x =0; x<5; x++) {
        ...
        for(int i=0; i<5; i++) {
            ...
            std::cout << "-----------------------------" <<std::endl;
            realWordTest(base_length * std::pow(2, i), files);
        }
        ...
        for(int i=0; i<5; i++) {
            ...
            std::cout << "ELEMENT COUNT: " << (base_length * std::pow(2, i)) <<std::endl;
            std::cout << "-----------------------------" <<std::endl;
            realWordTest(base_length * std::pow(2, i), files, 1);
        }
        ...
        for(int i=0; i<5; i++) {
            ...
            std::cout << "ELEMENT COUNT: " << (base_length * std::pow(2, i)) <<std::endl;
            std::cout << "-----------------------------" <<std::endl;
            realWordTest(base_length * std::pow(2, i), files, 2);
        }
    }
